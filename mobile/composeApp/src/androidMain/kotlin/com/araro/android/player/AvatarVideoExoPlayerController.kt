...
fun rememberAvatarVideoExoPlayerController(
        ...
        if (videoUrl.isNullOrBlank()) {
            ...
            exoPlayer.addListener(object : Player.Listener {
                override fun onPlaybackStateChanged(playbackState: Int) {
                    when (playbackState) {
                        Player.STATE_READY -> { if (!isPlaying) exoPlayer.play() }
                        Player.STATE_ENDED -> {
                            isPlaying = false
                            progress = 1f
    ...
    return AvatarVideoControllerResult(
        controller = object : StoryPlaybackController {
            ...
            override fun rewind() {
                player?.let { it.seekTo((it.currentPosition - AraroConstants.SEEK_MS).coerceAtLeast(0L)) }
            }
            override fun fastForward() {
                player?.let { val d = it.duration.coerceAtLeast(1L); it.seekTo((it.currentPosition + AraroConstants.SEEK_MS).coerceAtMost(d)) }
            }
            override fun setSleepTimer(minutes: Int) {
                sleepTimerJob?.cancel()
                if (minutes <= 0) return
