 ...
 * Never log secrets, tokens, or PII; use TamixaLog.maskUrl / TamixaLog.maskLast4 for identifiers.
 */
internal actual fun platformLog(level: String, tag: String, message: String, throwable: Throwable?) {
    val line = "Tamixa[$level]/$tag: $message"
    println(line)
    throwable?.let {
        val err = "${it::class.simpleName ?: "Throwable"}: ${it.message ?: ""}"
        println("Tamixa[$level]/$tag: $err")
