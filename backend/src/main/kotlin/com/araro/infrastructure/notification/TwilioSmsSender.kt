...
class TwilioSmsSender(
    ...
    override fun sendOtp(toPhone: String, code: String): Boolean {
        if (fromNumber.isBlank()) {
            log.info("Twilio from-number not configured; logging OTP for dev: To=$toPhone | Code=$code")
            return true
        }
        return try {
            val body = LinkedMultiValueMap<String, String>().apply {
                add("To", toPhone)
                add("From", fromNumber)
                add("Body", "Your Araro login code is: $code. Valid for 5 minutes.")
            }
            ...
            log.warn("Failed to send OTP SMS to $toPhone: ${e.message}")
            false
        }
    }
}
