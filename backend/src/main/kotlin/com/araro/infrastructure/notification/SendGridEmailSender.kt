...
class SendGridEmailSender(
    ...
    @Value("\${app.email.from-email:noreply@araro.com}") private val fromEmail: String,
    @Value("\${app.email.from-name:Araro}") private val fromName: String
) : EmailSenderPort {
    ...
    override fun sendMagicLinkOrCode(toEmail: String, magicLink: String, shortCode: String): Boolean {
        return try {
            val payload = mapOf(
                ...
                "subject" to "Your Araro login code",
                "content" to listOf(
                    mapOf(
                        "type" to "text/plain",
