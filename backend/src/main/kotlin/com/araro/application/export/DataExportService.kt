...
import com.araro.infrastructure.persistence.DataExportJobEntity
import com.araro.infrastructure.persistence.DataExportJobJpaRepository
...
class DataExportService(
    ...
    private val childJpaRepository: com.araro.infrastructure.persistence.ChildJpaRepository,
    ...
    private fun processJob(job: DataExportJobEntity) {
        ...
        val payload = DataExportPayload(
            ...
            children = childJpaRepository.findByParent_Id(parent.id).map { toChildExport(it) },
    ...
    private fun toChildExport(c: ChildEntity) = ChildExport(
        id = c.id,
        name = c.name,
        dateOfBirth = c.dateOfBirth.toString(),
        languagePreference = c.languagePreference,
        interests = c.interests,
        createdAt = c.createdAt.atOffset(ZoneOffset.UTC).format(DateTimeFormatter.ISO_INSTANT),
        favoriteColor = c.favoriteColor,
        favoriteAnimal = c.favoriteAnimal,
        characterTraits = c.characterTraits,
        avatarChoice = c.avatarChoice
    )
