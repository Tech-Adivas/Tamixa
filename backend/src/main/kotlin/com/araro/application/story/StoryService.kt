...
import com.araro.application.port.StoryCachePort
...
private const val MAX_READING_MINUTES = 5.0
...
class StoryService(
    ...
    fun findByParentAndId(parentEmail: String, storyId: Long): Story {
        ...
        val story = storyRepository.findById(storyId)
            ?: throw com.araro.application.admin.StoryNotFoundException(storyId)
    ...
    /**
     * Phase 2: Resolve child profile for personalization.
     */
    private fun resolveChildContext(parentId: Long, childId: Long?, requestAge: Int, requestLang: String): ChildContext {
        val fallback = ChildContext(requestAge, requestLang, null, null, null, null, null)
        if (childId == null) return fallback
        val child = childRepository.findById(childId) ?: return fallback
        if (child.parentId != parentId) return fallback
        val age = child.ageInYears().coerceIn(1, 12)
        val lang = child.languagePreference?.takeIf { it.isNotBlank() } ?: requestLang
        return ChildContext(
            age = age,
            language = lang,
    ...
    /**
     * Server-side enforcement: validate subscription state, then enforce FREE plan limit.
    ...
    private fun enforceStoryEntitlement(parentId: Long) {
        ...
        if (sub.plan != SubscriptionPlan.FREE) return  // Paid plans: unlimited
        val limit = appProperties.subscription.freeStoriesPerMonth
        val currentMonth = YearMonth.now(ZoneOffset.UTC).toString()
    ...
    private fun enforceReadingTimeLimit(wordCount: Int, readingTimeMinutes: Double) {
        if (readingTimeMinutes > MAX_READING_MINUTES) {
            throw StoryTooLongException(
                "Story exceeds 5 minutes reading time (${"%.1f".format(readingTimeMinutes)} min, $wordCount words). Max $MAX_WORDS words."
            )
        }
    }
}
