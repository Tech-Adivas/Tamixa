package com.tamixa.application.achievement

import com.tamixa.infrastructure.persistence.ChildAchievementEntity
import com.tamixa.infrastructure.persistence.ChildAchievementJpaRepository
...
class AchievementService(
    ...
    /** Parent-level achievements: earned when parent's story_completed count meets requirement. */
    @Transactional(readOnly = true)
    fun getByParentId(parentId: Long): List<AchievementDto> {
        val completions = storyAnalyticsJpaRepository.countByParent_IdAndEventType(parentId, "story_completed")
        return definitions.map { def ->
            AchievementDto(
                type = def.type,
                name = def.name,
                description = def.description,
                requiredCompletions = def.requiredCompletions,
                earned = completions >= def.requiredCompletions,
                earnedAt = null
            )
        }
    }
