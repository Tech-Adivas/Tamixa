...
class FamilyAvatarService(
    ...
    @Autowired(required = false) private val s3SignedUrlGenerator: com.tamixa.infrastructure.cdn.S3SignedUrlGenerator?,
) {
    ...
    /**
     * Admin-only: upload avatar for a parent by ID. Skips premium check so admins can test for any parent.
     */
    @Transactional
    fun uploadAvatarForAdmin(parentId: Long, imageBytes: ByteArray, contentType: String): ParentAvatar {
        ...
        val existing = parentAvatarRepository.findByParentId(parentId)
        if (existing != null) {
            familyAvatarStorage.delete(existing.storagePath)
        }
        val storagePath = familyAvatarStorage.upload(parentId, imageBytes, normalizedContentType)
        val now = Instant.now()
        val avatar = ParentAvatar(
            id = existing?.id ?: 0L,
            parentId = parentId,
        ...
        val saved = parentAvatarRepository.save(avatar)
        log.info("Avatar uploaded (admin) parentId={} path={}", parentId, storagePath)
        return saved
    }

    /**
     * Admin-only: delete avatar for a parent by ID. Skips premium check.
     */
    @Transactional
    fun deleteAvatarForAdmin(parentId: Long) {
        val existing = parentAvatarRepository.findByParentId(parentId)
            ?: throw AvatarNotFoundException("No avatar to delete")
        familyAvatarStorage.delete(existing.storagePath)
        parentAvatarRepository.deleteByParentId(parentId)
        log.info("Avatar deleted (admin) parentId={}", parentId)
    }
