...
class NarrationGeminiAdapter(
    ...
    @Value("\${app.narration.max-narration-tokens:16384}") private val configuredMaxTokens: Int,
    ...
    override fun formatNarration(
        ...
        if (appProperties.llm.gemini.apiKey.isBlank()) {
            throw IllegalStateException("Gemini API key not configured. Set GEMINI_API_KEY.")
        }
        val effectiveMax = maxTokens.coerceIn(256, 16384).coerceAtMost(configuredMaxTokens.coerceIn(256, 16384))
        val userPrompt = buildUserPrompt(storyText, age, toneMode, language)
        val (content, usage) = try {
            ...
            throw NarrationLLMException("Rewrite failed: ${e.message}")
        } catch (e: Exception) {
            throw NarrationLLMException("Rewrite failed: ${e.message}")
        }
        log.debug(
            "Narration Gemini response tokens: prompt={}, completion={}",
            usage?.promptTokens,
    ...
    override fun transformWithCustomPrompt(
        ...
        val (content, usage) = try {
            ...
            throw NarrationLLMException("Custom prompt transform failed: ${e.message}")
        } catch (e: Exception) {
            throw NarrationLLMException("Custom prompt transform failed: ${e.message}")
        }
        return NarrationLLMResult(
