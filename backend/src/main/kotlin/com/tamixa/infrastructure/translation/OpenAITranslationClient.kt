...
class OpenAITranslationClient(
    ...
    override fun translateStructured(
        ...
        val systemPrompt = """
            ...
            CRITICAL: Preserve all inline narration markers exactly as-is in the content. These include [Pause 500ms], [Pause 1s], [Happy tone], [Soft voice], [Warm tone], [Calm], [Whisper], [Excited]. Do NOT remove, translate, or alter them—they are TTS instructions.
    ...
    private fun parseTranslationResponse(
        ...
        return try {
            ...
            val rawTitle = sanitizeTranslationText(node["title"]?.takeIf { !it.isNull }?.asText()?.takeIf { it.isNotBlank() })
            val rawMoral = sanitizeTranslationText(node["moral"]?.takeIf { !it.isNull }?.asText()?.takeIf { it.isNotBlank() })
            // When translating FROM Tamil TO another language: reject title/moral that still contain Tamil (LLM failed to translate)
            val (finalTitle, finalMoral) = if (sourceLang == targetLang) {
                (rawTitle ?: originalTitle) to (rawMoral ?: originalMoral)
            } else if (sourceLang == "ta") {
                val t = if (rawTitle != null && containsTamilScript(rawTitle)) {
                    log.warn("Translation title contains Tamil script for target={}; rejecting (LLM did not translate)", targetLang)
                    null
                } else rawTitle
                val m = if (rawMoral != null && containsTamilScript(rawMoral)) {
                    log.warn("Translation moral contains Tamil script for target={}; rejecting (LLM did not translate)", targetLang)
                    null
                } else rawMoral
                t to m
            } else {
