...
class SSMLEmotionMappingTest {
    ...
    @Test
    fun `buildSSMLFromEmotionTagged adds pitch for DIALOGUE`() {
    ...
    @Test
    fun `buildSSMLFromEmotionTagged adds volume soft for WHISPER`() {
        val tagged = EmotionTaggedScript(
            originalScript = "She whispered",
            segments = listOf(EmotionSegment("She whispered", EmotionTag.WHISPER)),
            wordCount = 2
        )
        val result = ssmlBuilder.buildSSMLFromEmotionTagged(tagged, "en", 5, ToneMode.CALM)
    ...
    @Test
    fun `buildSSMLFromEmotionTagged uses Tamil slower rate`() {
        val tagged = EmotionTaggedScript(
        ...
        assertTrue(result.contains("rate=\"") && result.contains("ta-IN"))
    }

    @Test
    fun `buildSSMLFromEmotionTagged adds pitch for CONVERSATIONAL`() {
        val tagged = EmotionTaggedScript(
