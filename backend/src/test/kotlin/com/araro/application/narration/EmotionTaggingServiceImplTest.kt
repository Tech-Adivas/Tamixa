...
import com.araro.application.narration.impl.EmotionTaggingServiceImpl
...
class EmotionTaggingServiceImplTest {
    ...
    fun `validateWordCountTolerance rejects large deviation`() {
        val result = com.araro.domain.narration.EmotionTaggedScript(
            originalScript = "a b c",
            segments = listOf(
                com.araro.domain.narration.EmotionSegment("a", EmotionTag.CALM),
                com.araro.domain.narration.EmotionSegment("b", EmotionTag.CALM)
            ),
            wordCount = 3
        )
