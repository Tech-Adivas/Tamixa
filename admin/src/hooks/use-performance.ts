...
/** Chromium / spec-adjacent navigator fields not always in lib.dom. */
type NavigatorDeviceHints = Navigator & {
  deviceMemory?: number;
...
type PerformanceWithOptionalMemory = Performance & { memory?: ChromePerformanceMemory };
...
export function useDeviceCapabilities() {
  ...
  useEffect(() => {
    const nav = globalThis.navigator as NavigatorDeviceHints;
...
/**
 * Hook for adaptive table/list rendering (P0 enhancement)
 */
export function useAdaptiveRendering() {
  ...
  return useMemo(() => ({
    ...
    getChartDataPoints: (defaultPoints: number): number => {
      if (isLowEnd) {
        return Math.max(10, Math.floor(defaultPoints * 0.5)); // 50% reduction, minimum 10
      }
...
export function usePerformanceMonitoring() {
  ...
  useEffect(() => {
    ...
    const updateMemoryUsage = () => {
      const memory = (globalThis.performance as PerformanceWithOptionalMemory).memory;
      if (memory) {
        setMetrics(prev => ({
          ...prev,
